...
class LocalParticipant extends Participant<LocalTrackPublication> {
  ...
  Future<LocalTrackPublication<LocalVideoTrack>> publishVideoTrack(
    ...
    if (kIsWeb || lkPlatformIsMobile()) {
      ...
      try {
        ...
        if ((settings['width'] is int && settings['width'] as int > 0) && (settings['height'] is int && settings['height'] as int > 0)) {
    ...
    Future<lk_models.TrackInfo> negotiate() async {
      ...
      if (kIsWeb && lkBrowser() == BrowserType.firefox && track.kind == TrackType.AUDIO) {
        ...
        room.engine.publisher?.setTrackBitrateInfo(TrackBitrateInfo(cid: track.getCid(), transceiver: track.transceiver, codec: publishOptions.videoCodec, maxbr: encodings![0].maxBitrate! ~/ 1000));
      }
    ...
    final req = lk_rtc.AddTrackRequest(
      cid: track.getCid(),
      name: publishOptions.name ?? (track.source == TrackSource.screenShareVideo ? VideoPublishOptions.defaultScreenShareName : VideoPublishOptions.defaultCameraName),
      type: track.kind.toPBType(),
      source: track.source.toPBType(),
      encryption: room.roomOptions.lkEncryptionType,
  ...
  @override
  List<LocalTrackPublication<LocalVideoTrack>> get videoTrackPublications => trackPublications.values.whereType<LocalTrackPublication<LocalVideoTrack>>().toList();

  /// A convenience property to get all audio tracks.
  @override
  List<LocalTrackPublication<LocalAudioTrack>> get audioTrackPublications => trackPublications.values.whereType<LocalTrackPublication<LocalAudioTrack>>().toList();
  ...
  Future<LocalTrackPublication?> setScreenShareEnabled(bool enabled, {bool? captureScreenAudio, ScreenShareCaptureOptions? screenShareCaptureOptions}) async {
    screenShareCaptureOptions ??= room.roomOptions.defaultScreenShareCaptureOptions;
    return setSourceEnabled(TrackSource.screenShareVideo, enabled, captureScreenAudio: captureScreenAudio, screenShareCaptureOptions: screenShareCaptureOptions);
  }
  ...
  Future<LocalTrackPublication?> setSourceEnabled(TrackSource source, bool enabled,
      {bool? captureScreenAudio, AudioCaptureOptions? audioCaptureOptions, CameraCaptureOptions? cameraCaptureOptions, ScreenShareCaptureOptions? screenShareCaptureOptions}) async {
    ...
    if (publication != null) {
      ...
      if (source == TrackSource.camera) {
        ...
        AudioCaptureOptions captureOptions = audioCaptureOptions ?? room.roomOptions.defaultAudioCaptureOptions;
        final track = await LocalAudioTrack.create(captureOptions);
        return await publishAudioTrack(track);
      } else if (source == TrackSource.screenShareVideo) {
        ScreenShareCaptureOptions captureOptions = screenShareCaptureOptions ?? room.roomOptions.defaultScreenShareCaptureOptions;
  ...
  @internal
  Iterable<lk_rtc.TrackPublishedResponse> publishedTracksInfo() => trackPublications.values.map((e) => e.toPBTrackPublishedResponse());
  ...
  Future<void> publishAdditionalCodecForPublication(
    ...
    var layers = Utils.computeVideoLayers(dimensions, encodings, isSVCCodec(backupCodec));
    ...
    var req = lk_rtc.AddTrackRequest(
      cid: cid,
      name: options.name ?? (track.source == TrackSource.screenShareVideo ? VideoPublishOptions.defaultScreenShareName : VideoPublishOptions.defaultCameraName),
      type: track.kind.toPBType(),
      source: track.source.toPBType(),
      layers: layers,
      sid: publication.sid,
...
extension RPCMethods on LocalParticipant {
  ...
  Future<void> publishRpcRequest({
    ...
    if ((room.engine.serverInfo?.version.isNotEmpty ?? false) && compareVersions(room.engine.serverInfo!.version, '1.8.0') < 0) {
  ...
  Future<void> handleIncomingRpcRequest(
    ...
    try {
      ...
      final response = await handler(RpcInvocationData(requestId: requestId, callerIdentity: callerIdentity, payload: payload, responseTimeoutMs: responseTimeoutMs.toInt()));
      ...
      if (error is RpcError) {
        ...
        logger.warning('Uncaught error returned by RPC handler for ${method}. Returning RpcError.applicationError instead. $error');
  ...
  Future<String> performRpc(PerformRpcParams params) async {
    ...
    try {
      await publishRpcRequest(
        ...
        responseTimeout: Duration(milliseconds: params.responseTimeoutMs.inMilliseconds - maxRoundTripLatency.inMilliseconds),
        version: kRpcVesion,
      );
  ...
  Future<TextStreamWriter> streamText(StreamTextOptions? options) async {
    ...
    final header = lk_models.DataStream_Header(
      ...
      textHeader: lk_models.DataStream_TextHeader(
        ...
        operationType: options?.type == 'update' ? lk_models.DataStream_OperationType.UPDATE : lk_models.DataStream_OperationType.CREATE,
      ),
    );
    ...
    final writableStream = WritableStream<String>(destinationIdentities: destinationIdentities!, engine: room.engine, streamId: streamId);

    onEngineClose() async {
      await writableStream.close();
    }

    var cancelFun = room.engine.events.once<EngineClosingEvent>((_) => onEngineClose);
